...
export const ValidateMixinImplementation = (superclass) =>
    ...
    class ValidateMixin extends FormControlMixin(
        ...
        static get scopedElements() {
            const scopedElementsCtor = super.constructor;

            return {
                ...scopedElementsCtor.scopedElements,
                "vhaa-validation-feedback": ValidationFeedback,
            };
        }
        ...
        get slots() {
            ...
            return {
                ...super.slots,
                feedback: () => {
                    const feedbackEl = this.createScopedElement("validation-feedback");
                    feedbackEl.setAttribute("data-tag-name", "validation-feedback");
                    return feedbackEl;
                },
            };
        }
        ...
        constructor() {
            ...
            /**
             * The amount of feedback messages that will visible in vhaaValidationFeedback
             * @configurable
             * @protected
             */
        ...
        updateSync(name, oldValue) {
            ...
            if (name === "shouldShowFeedbackFor") {
                ...
                if (diff.length > 0) {
                    this.dispatchEvent(
                        new Event(`shouldShowFeedbackForChanged`, { bubbles: true })
                    );
                }
            }
        }
        ...
        async __executeValidators() {
            ...
            const value =
                this.modelValue instanceof Unparseable
             ...
             */
            // TODO: Try to remove this when we have a single vhaa form core package, because then we can
        ...
        __executeResultValidators(regularValidationResult) {
            ...
            return resultValidators
                .map((v) => ({
                    ...
                    outcome: /** @type {boolean|string} */ (
                        v.executeOnResults({
                            regularValidationResult: regularValidationResult.map(
                                (entry) => entry.validator
                            ),
                            prevValidationResult: this.__prevValidationResult.map(
                                (entry) => entry.validator
                            ),
        ...
        __finishValidation({ source, hasAsync }) {
            ...
            /** private event that should be listened to by vhaaFieldSet */
        ...
        __setupValidators() {
            ...
            this._allValidators.forEach((v) => {
                if (!(v instanceof Validator)) {
                    ...
                    const errorMessage = `Validators array only accepts class instances of Validator. Type "${errorType}" found. This may be caused by having multiple installations of @vhaa/form-core.`;
         ...
         *
         * For `._feedbackNode` (extension of vhaaValidationFeedback):
        ...
        updated(changedProperties) {
            ...
            if (
                changedProperties.has("__childModelValueChanged") &&
                ...
                if (prevStates) {
                    Object.entries(this.validationStates).forEach(([type, feedbackObj]) => {
                        if (
                            ...
                            this.dispatchEvent(
                                new CustomEvent(`${type}StateChanged`, { detail: feedbackObj })
                            );
                        }
                    });
                }
        ...
        _updateShouldShowFeedbackFor() {
            ...
            if (
                ...
                JSON.stringify(newShouldShowFeedbackFor)
            ) {
