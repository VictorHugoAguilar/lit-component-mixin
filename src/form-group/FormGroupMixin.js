 ...
 * Also, it is th basis of the VhaaFieldset element (which supports name based retrieval of
    ...
    class FormGroupMixin extends FormRegistrarMixin(
        ...
        _getFromAllFormElements(property, filterFn) {
            ...
            this.formElements._keys().forEach((name) => {
                ...
                if (elem instanceof FormControlsCollection) {
                    result[name] = elem
                        .filter((el) => _filterFn(el, property))
                        .map((el) => el[property]);
        ...
        _anyFormElementHas(property) {
            return Object.keys(this.formElements).some((name) => {
                if (Array.isArray(this.formElements[name])) {
                    return this.formElements[name].some(
                        (/** @type {FormControl} */ el) => !!el[property]
                    );
                }
        ...
        _everyFormElementHas(property) {
            return Object.keys(this.formElements).every((name) => {
                if (Array.isArray(this.formElements[name])) {
                    return this.formElements[name].every(
                        (/** @type {FormControl} */ el) => !!el[property]
                    );
                }
                return !!this.formElements[name][property];
         ...
         * @example
         * ```html
         * <Vhaa-fieldset name="address">
         *   <Vhaa-input name="street" label="Street" .modelValue="${'Park Avenue'}"></Vhaa-input>
         *   <Vhaa-input name="number" label="Number" .modelValue="${100}">...</Vhaa-input>
        ...
        __storeAllDescriptionElementsInParentChain() {
            ...
            while (parent) {
                ...
                const orderedEls = getAriaElementsInRightDomOrder(descriptionElements, {
                    reverse: true,
                });
