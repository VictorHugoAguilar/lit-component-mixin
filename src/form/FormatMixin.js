...
/**
 * @desc Designed to be applied on top of a VhaaField.
 ...
 * Another difference is that formattedValue lives on `VhaaField`, whereas viewValue is shared
 * across `VhaaField` and `._inputNode`.
 *
...
const FormatMixinImplementation = (superclass) =>
    ...
    class FormatMixin extends ValidateMixin(FormControlMixin(superclass)) {
        ...
        /**
         * Synchronization from `._inputNode.value` to `VhaaField` (flow [2])
         * Downwards syncing should only happen for `VhaaField`.value changes from 'above'.
        ...
        /**
         * Synchronization from `VhaaField.value` to `._inputNode.value`
        ...
        _onUserInputChanged() {
            ...
            // `VhaaField`, to be able to act on (imperatively set) value changes
        ...
        connectedCallback() {
            ...
            // `VhaaField`(recommended api) as the api (this is a downwards sync).
            // However, when no value is specified on `VhaaField`, have support for sync of the real
            // input to the `vhaaField` (upwards sync).
