...
import { SlotMixin, ScopedElementsMixin } from "../index.js";
...
describe("SlotMixin", () => {
    ...
    it("does not override user provided slots", async () => {
        ...
        expect(/** @type HTMLParagraphElement */ (el.children[0]).innerText).to.equal(
            "user-content"
        );
    });
    ...
    it("does add when user provided slots are not direct children", async () => {
        ...
        const el = await fixture(
            `<${tag}><p><span slot="content">user-content</span></p></${tag}>`
        );
